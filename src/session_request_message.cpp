...
namespace fscp
{
	size_t session_request_message::write(void* buf, size_t buf_len)
	{
		...
		message::write(buf, buf_len, CURRENT_PROTOCOL_VERSION, MESSAGE_TYPE_SESSION_REQUEST, BODY_LENGTH);

		return HEADER_LENGTH + BODY_LENGTH;
	}

	session_request_message::session_request_message(const void* buf, size_t buf_len) :
		message(buf, buf_len)
	{
